...
type CollectorLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha.Collector, err error)
...
// List lists all Collectors in the indexer.
func (s *collectorLister) List(selector labels.Selector) (ret []*v1alpha.Collector, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha.Collector))
	})
	return ret, err
}
...
type CollectorNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha.Collector, error)
...
// List lists all Collectors in the indexer for a given namespace.
func (s collectorNamespaceLister) List(selector labels.Selector) (ret []*v1alpha.Collector, err error) {
...
// Get retrieves the Collector from the indexer for a given namespace and name.
func (s collectorNamespaceLister) Get(name string) (*v1alpha.Collector, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha.Resource("collector"), name)
	}
	return obj.(*v1alpha.Collector), nil
}
